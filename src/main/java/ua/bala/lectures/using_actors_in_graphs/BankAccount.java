    ...
    public static void main(String[] args) {
        ...
        Sink<Transfer, CompletionStage<Done>> transferLogger = Sink.foreach( transfer -> {
            System.out.println("Transfer from " + transfer.getFrom().getAccountNumber() +
                " to " + transfer.getTo().getAccountNumber() +
                " of " + transfer.getTo().getAmount());
        });
        ...
        Flow<Transaction, AccountManager.AddTransactionResponse, NotUsed> attemptToApplyTransaction = ActorFlow.ask(accountManager, Duration.ofSeconds(10), AccountManager.AddTransactionCommand::new);
        ...
        Flow<AccountManager.AddTransactionResponse, AccountManager.AccountManagerCommand, NotUsed> receiveResult = Flow.of(AccountManager.AddTransactionResponse.class)
            .map( result -> {
        ...
        Sink<AccountManager.AccountManagerCommand, NotUsed> displayBalanceSink = ActorSink.actorRef(accountManager, new AccountManager.CompleteCommand(), (throwable) -> new AccountManager.FailedCommand());

        Source<Transaction, NotUsed> newSource = Source.fromGraph(sourcePartialGraph);

        newSource
            .via(attemptToApplyTransaction
